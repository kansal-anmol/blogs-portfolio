...
```typescript
type Action = { type: 'A'; payload: { body: string } } | { type: 'B'; payload: { title: string } };
...
That means TypeScript treats the handler and the payload as **unrelated**, even if you _just_ narrowed `action.type` a few lines earlier.
